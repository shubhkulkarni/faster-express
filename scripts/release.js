...
async function main() {
  ...
  try {
    ...
    if (branch !== 'main' && branch !== 'master') {
      console.log(`❌ You're on branch '${branch}'. Please switch to main/master branch first.`);
      process.exit(1);
    }
  ...
  try {
    ...
    if (status.trim()) {
      console.log('❌ Working directory is not clean. Please commit or stash your changes.');
      process.exit(1);
    }
  ...
  const versionType = await askQuestion('Select version bump type (patch/minor/major): ');
  ...
  const publish = await askQuestion('Publish to NPM after version bump? (y/N): ');
  const shouldPublish = publish.toLowerCase() === 'y' || publish.toLowerCase() === 'yes';
  ...
  try {
    ...
    if (shouldPublish) {
      ...
      console.log(`📦 Install with: npm install -g faster-express@${newVersion}`);
    } else {
