...
import { AddOptions, BoilerplateLevel, SwaggerTheme } from "../types";
...
export async function addResource(resourceName: string, options: AddOptions) {
  ...
  try {
    ...
    console.log(
      chalk.cyan(
        `  src/resources/${resourceName}/controller.${projectConfig.language}`
      )
    );
    console.log(
      chalk.cyan(
        `  src/resources/${resourceName}/service.${projectConfig.language}`
      )
    );
    console.log(
      chalk.cyan(
        `  src/resources/${resourceName}/routes.${projectConfig.language}`
      )
    );
    console.log(
      chalk.cyan(
        `  src/resources/${resourceName}/validation.${projectConfig.language}`
      )
    );
    console.log(
      chalk.cyan(
        `  src/resources/${resourceName}/index.${projectConfig.language}`
      )
    );
    ...
    if (projectConfig.database && projectConfig.database !== "none") {
      console.log(
        chalk.cyan(
          `  src/resources/${resourceName}/model.${projectConfig.language}`
        )
      );
    }
    ...
    if (resourceConfig.generateTests) {
      console.log(
        chalk.cyan(
          `  src/resources/${resourceName}/${resourceName}.test.${projectConfig.language}`
        )
      );
    }
...
function detectDatabase(
  packageJson: any
): "mongodb" | "postgres" | "none" | undefined {
...
function detectORM(
  packageJson: any
): "mongoose" | "prisma" | "sequelize" | "typeorm" | undefined {
  const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
