...
export async function listResources() {
  ...
  try {
    ...
    if (!await fs.pathExists(resourcesPath)) {
      console.log(chalk.yellow('📭 No resources directory found. Use "faster-express add <resource-name>" to create your first resource.'));
      return;
    }
    ...
    if (resourceDirs.length === 0) {
      console.log(chalk.yellow('📭 No resources found. Use "faster-express add <resource-name>" to create your first resource.'));
      return;
    }
    ...
    console.log(chalk.cyan('  faster-express add <resource>    # Add a new resource'));
    console.log(chalk.cyan('  faster-express remove <resource> # Remove a resource'));
