  ...
  const answer = await inquirer.prompt([
    {
      ...
      name: "configureMore",
      message:
        "⚙️  Do you want to configure additional options (database, auth, testing, etc.)?",
      default: true,
    },
  ]);
...
function createDefaultConfig(
  ...
  return {
    ...
    light: false,
    boilerplateLevel:
      (options.boilerplate as "minimal" | "full" | "signatures") || "full",
...
async function promptFullConfiguration(
  projectName: string,
  options: CreateOptions
): Promise<ProjectConfig> {
  ...
  if (!options.boilerplate) {
    questions.push({
      ...
      choices: [
        {
          name: "Full - Complete implementation with all features",
          value: "full",
        },
        {
          name: "Signatures - Method signatures only, implement yourself",
          value: "signatures",
        },
  ...
  if (options.withSwagger === undefined) {
    questions.push({
      type: "confirm",
      name: "enableSwagger",
  ...
  questions.push({
    type: "input",
    name: "swaggerTitle",
    message: "What should be the API documentation title?",
    default: `${projectName} API`,
    when: (answers: any) => options.withSwagger || answers.enableSwagger,
  });

  questions.push({
    type: "input",
    name: "swaggerPath",
    message: "What path should serve the API documentation?",
    default: "/docs",
    when: (answers: any) => options.withSwagger || answers.enableSwagger,
  });
  ...
  questions.push({
    type: "list",
    name: "swaggerTheme",
    message: "Which Swagger UI theme would you prefer?",
    choices: [
      { name: "Default - Clean and professional", value: "default" },
      { name: "Dark - Dark mode theme", value: "dark" },
      { name: "Material - Material design theme", value: "material" },
    ],
    default: "default",
    when: (answers: any) => options.withSwagger || answers.enableSwagger,
  });
  ...
  questions.push({
    type: "confirm",
    name: "swaggerExamples",
    message: "Generate example request/response schemas in documentation?",
    default: true,
    when: (answers: any) => options.withSwagger || answers.enableSwagger,
  });
  ...
  // Process swagger configuration
  const swaggerEnabled = options.withSwagger || answers.enableSwagger || false;
  const swaggerConfig = {
    enabled: swaggerEnabled,
    title: swaggerEnabled
      ? options.swaggerTitle || answers.swaggerTitle || `${projectName} API`
      : undefined,
    description: swaggerEnabled
      ? `API documentation for ${projectName}`
      : undefined,
    version: swaggerEnabled ? "1.0.0" : undefined,
    path: swaggerEnabled
      ? options.swaggerPath || answers.swaggerPath || "/docs"
      : undefined,
    theme: swaggerEnabled ? answers.swaggerTheme || "default" : undefined,
    generateExamples: swaggerEnabled
      ? answers.swaggerExamples !== false
      : undefined,
  };
  ...
  return {
    ...
    packageManager:
      (options.pkgManager as "npm" | "yarn" | "pnpm") ||
      answers.packageManager ||
      "npm",
    style:
      (options.style as "resource" | "layered") || answers.style || "resource",
    database,
    orm:
      (options.orm as "mongoose" | "prisma" | "sequelize" | "typeorm") ||
      answers.orm,
    auth,
    testing:
      options.tests === false
        ? false
        : options.withJest || answers.testing || false,
    ...
    light: false,
    boilerplateLevel:
      (options.boilerplate as "minimal" | "full" | "signatures") ||
      answers.boilerplateLevel ||
      "full",
    includeValidation:
      options.validation !== false
        ? answers.includeValidation !== false
        : false,
...
export async function promptResourceConfig(
  ...
  withAuth: boolean;
  boilerplateLevel: "minimal" | "full" | "signatures";
  ...
  questions.push({
    ...
    message:
      "⚙️  Do you want to configure additional options for this resource?",
    default: false,
  });
  ...
  questions.push({
    ...
    default: true,
    when: (answers: any) =>
      answers.configureOptions && options.validation === undefined,
  });
  ...
  questions.push({
    ...
    message:
      'Custom endpoints (comma-separated, e.g., "activate,deactivate,bulk-update")?',
  ...
  questions.push({
    ...
    default: true,
    when: (answers: any) =>
      answers.configureOptions && options.tests === undefined,
  });
  ...
  questions.push({
    ...
    default: false,
    when: (answers: any) =>
      answers.configureOptions && options.withAuth === undefined,
  });
  ...
  return {
    name: resourceName,
    generateTests:
      options.tests !== false ? answers.generateTests !== false : false,
    ...
    boilerplateLevel:
      ...
      "full",
    includeValidation:
      options.validation !== false ? answers.includeValidation !== false : true,
