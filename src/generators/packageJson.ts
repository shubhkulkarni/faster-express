...
export async function generatePackageJson(config: ProjectConfig, projectPath: string): Promise<void> {
  const packageJson = {
    name: config.name,
    version: '1.0.0',
    description: '',
    ...
    devDependencies: config.language === 'ts' ? await generateDevDependencies(config) : {}
  };
...
function generateScripts(config: ProjectConfig): Record<string, string> {
  ...
  if (config.eslint) {
    scripts.lint = config.language === 'ts' ? 'eslint src/**/*.ts' : 'eslint src/**/*.js';
    scripts['lint:fix'] = config.language === 'ts' ? 'eslint src/**/*.ts --fix' : 'eslint src/**/*.js --fix';
  }

  if (config.prettier) {
    scripts.format = config.language === 'ts' ? 'prettier --write src/**/*.ts' : 'prettier --write src/**/*.js';
  }
...
async function generateDependencies(config: ProjectConfig): Promise<Record<string, string>> {
...
async function generateDevDependencies(config: ProjectConfig): Promise<Record<string, string>> {
